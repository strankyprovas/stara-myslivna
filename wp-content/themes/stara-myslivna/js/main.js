...
(function() {
    ...
    function updateLightbox() {
        ...
        // Item může být <a> (přímo) nebo <figure>/<div> obsahující <a>
        var href = item.href;
    ...
    document.addEventListener('click', function(e) {
        // Klik na .gallery-item (galerie) NEBO .chef-photo (Zajímavosti/Karty/Šéfkuchaři – foto vlevo, text vpravo)
        var a = e.target.closest('.gallery-item, .photo-gallery .gallery-item, .chef-photo');
        if (!a) return;
        e.preventDefault();

        // Chef-photo struktura (Zajímavosti, Karty…) – sbírej všechny .chef-photo na stránce
        if (a.classList.contains('chef-photo')) {
            var entry = a.closest('.entry-content') || document;
            var chefItems = Array.from(entry.querySelectorAll('.chef-photo'));
            var chefIndex = chefItems.indexOf(a);
            showLightbox(chefItems, chefIndex < 0 ? 0 : chefIndex);
            return;
        }

        // Skupina dle data-lightbox, jinak dle nejbližší .gallery rodičovského elementu
        var group = a.dataset.lightbox;
        var allItems;
        if (group) {
            allItems = Array.from(document.querySelectorAll('.gallery-item[data-lightbox="' + group + '"]'));
        } else {
            var galleryParent = a.closest('.gallery, .photo-gallery, .entry-content');
            allItems = galleryParent
                ? Array.from(galleryParent.querySelectorAll('.gallery-item'))
                : [a];
        }
        var index = allItems.indexOf(a);
        showLightbox(allItems, index < 0 ? 0 : index);
    });
