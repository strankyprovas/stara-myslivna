...
<div class="site-wrapper">
    ...
    <header class="site-header">
        ...
        <!-- Google Translate + social icons v jednom flex containeru -->
    ...
    function googleTranslateElementInit() {
        new google.translate.TranslateElement({
            pageLanguage: 'cs',
            includedLanguages: 'cs,en,de,ru,fr,it,pl,sk',
...
function myslivna_horiz_nav_fallback() {
    ...
    $nav_labels = ['O restauraci','Jídelní lístek','Menu skupiny','Rezervace','Služby','Akce','Suvenýry','Kontakty'];
    ...
    foreach ($nav_slugs as $i => $slug) {
        ...
        $children = get_pages(['parent'=>$page->ID,'sort_column'=>'menu_order','post_status'=>'publish']);
        ...
        // Top "Jídelní lístek" vede rovnou na CZ JL (klient 5.5.2026: zrušit úvodní stranu, klik = ihned JL)
        $top_url = get_permalink($page->ID);
        if ($slug === 'menu') {
            $jl_q = get_posts(['name'=>'jidelni-listek','post_type'=>'page','post_status'=>'publish','numberposts'=>1]);
            if ($jl_q) $top_url = get_permalink($jl_q[0]->ID);
        }
        echo '<li' . $cls . '>';
        echo '<a href="' . esc_url($top_url) . '">' . esc_html($nav_labels[$i]) . '</a>';
        if ($children) {
